...
| 항목 | 값 |
|------|-----|
| `APP_VERSION` (js/app.js) | `v5.47` |
| SW 캐시 버전 (sw.js) | `hanja-v70` |
| 최종 업데이트 | 2026-04-26 |
...
## 2. 프로젝트 개요
...
### 무한 복습 (Infinite Review) - 신규

- **목적**: 학습한 한자를 제한 없이 반복 학습하여 완전히 숙달함.
- **대상**: 1회 이상 학습한 모든 한자 (신규 한자 제외).
- **로직**: A(오답) → B(복습 주기) → D(대기) 순으로 무한 공급.
- **UI**: 세션 종료 화면 없이 즉시 다음 문제로 진행 (X 버튼으로 종료).
- **기록**: 매 문제 정답/오답 시 실시간으로 SRS 주기 및 통계 업데이트.
...
// js/data/levelN.js
{
  ...
  similar: [],          // 유의자 배열
  story: "물(氵)이 흐르는..."  // 연상 스토리 (전 급수 100% 완료)
}
```
...
- **학습 입문 도구**: 사전 학습 여부와 상관없이 모든 스토리 있는 카드 출제 가능
- **큐 구성**: 복습 대상(셔플) 우선 배치 + 부족분은 미학습 카드(순차)로 채움
- **학습 연동**: 퀴즈 진행 시 자동으로 '학습 시작'(`smState` 등록) 처리
...
| 동기부여 요소 | 문제 수 대신 XP 표시 (1문제 = 1 XP) |
