...
## 1. 개발 기본 원칙
- **버전 관리**: 새로운 기능 추가나 중요한 데이터 업데이트 시 `js/app.js`의 `APP_VERSION` 상수를 반드시 업데이트합니다. (현재 버전: v4.1)
...
- **세션 구성**: 
  - 기본 세션: '복습 카드(최대 10장)' + '신규 카드(최대 10장)'
  - 홈 화면의 통계 박스(복습, 신규)를 클릭하여 특정 모드의 세션만 시작할 수 있습니다.
- **제한(Cap)**: 학습 부하를 줄이기 위해 복습과 신규 카드 모두 세션당 최대 10장으로 제한합니다.
